 ...
 * </p>
 */
public class OptimizedThrowableProxy implements Serializable {
    ...
    private static final OptimizedThrowableProxy[] EMPTY_THROWABLE_PROXY_ARRAY = new OptimizedThrowableProxy[0];
    ...
    private final OptimizedThrowableProxy causeProxy;
    ...
    private final OptimizedThrowableProxy[] suppressedProxies;
    ...
    private OptimizedThrowableProxy() {
        this.throwable = null;
        this.name = null;
     ...
     */
    public OptimizedThrowableProxy(final Throwable throwable) {
        this(throwable, null);
    }
     ...
     */
    private OptimizedThrowableProxy(final Throwable throwable, final Set<Throwable> visited) {
...
//        final Map<String, CacheEntry> map = new HashMap<>();
        ...
        this.causeProxy = throwableCause == null ? null : new OptimizedThrowableProxy(throwable, stack, CACHE_ENTRY_MAP, throwableCause,
     ...
     */
    private OptimizedThrowableProxy(final Throwable parent, final Stack<Class<?>> stack, final Map<String, CacheEntry> map,
        ...
        this.causeProxy = causeCause == null || causeVisited.contains(causeCause) ? null : new OptimizedThrowableProxy(parent,
    ...
    public boolean equals(final Object obj) {
        ...
        final OptimizedThrowableProxy other = (OptimizedThrowableProxy) obj;
    ...
    private void formatCause(final StringBuilder sb, final String prefix, final OptimizedThrowableProxy cause,
    ...
    private void formatThrowableProxy(final StringBuilder sb, final String prefix, final String causeLabel,
                                      final OptimizedThrowableProxy throwableProxy, final List<String> ignorePackages,
    ...
    private void formatSuppressed(final StringBuilder sb, final String prefix, final OptimizedThrowableProxy[] suppressedProxies,
                                  final List<String> ignorePackages, final TextRenderer textRenderer, final String suffix) {
        if (suppressedProxies == null) {
            return;
        }
        for (final OptimizedThrowableProxy suppressedProxy : suppressedProxies) {
     ...
     * @param suffix
     */
    public void formatWrapper(final StringBuilder sb, final OptimizedThrowableProxy cause, final String suffix) {
    ...
    public void formatWrapper(final StringBuilder sb, final OptimizedThrowableProxy cause, final List<String> ignorePackages, final String suffix) {
     ...
     */
    @SuppressWarnings("ThrowableResultOfMethodCallIgnored")
    public void formatWrapper(final StringBuilder sb, final OptimizedThrowableProxy cause, final List<String> ignorePackages,
                              final TextRenderer textRenderer, final String suffix) {
    ...
    public OptimizedThrowableProxy getCauseProxy() {
     ...
     */
    public OptimizedThrowableProxy[] getSuppressedProxies() {
    ...
    public String getSuppressedStackTrace(final String suffix) {
        final OptimizedThrowableProxy[] suppressed = this.getSuppressedProxies();
        if (suppressed == null || suppressed.length == 0) {
            return Strings.EMPTY;
        }
        final StringBuilder sb = new StringBuilder("Suppressed Stack Trace Elements:").append(EOL);
        for (final OptimizedThrowableProxy proxy : suppressed) {
    ...
    private OptimizedThrowableProxy[] toSuppressedProxies(final Throwable thrown, Set<Throwable> suppressedVisited) {
        try {
            final Throwable[] suppressed = thrown.getSuppressed();
            if (suppressed == null) {
                return EMPTY_THROWABLE_PROXY_ARRAY;
            }
            final List<OptimizedThrowableProxy> proxies = new ArrayList<>(suppressed.length);
            ...
            for (int i = 0; i < suppressed.length; i++) {
                ...
                if (!suppressedVisited.contains(candidate)) {
                    ...
                    proxies.add(new OptimizedThrowableProxy(candidate, suppressedVisited));
                }
            }
            return proxies.toArray(new OptimizedThrowableProxy[proxies.size()]);
